...
1つの関数やファイルの実装が肥大化した際には分割が必要になりますが、関数やファイルの分割は読み手にコンテキストの切り替えを要求します。そのため、**どのような切り口で分割するか**が重要です。

## 切り口の例：ロジックとマークアップで分ける

Reactのhooksが登場して以来、「ロジックとマークアップを分離する」という考え方が広まりました。stateやイベントハンドラなどのロジックをカスタムhooksに切り出し、ReactNodeは別の場所に書くという方法です。
...
- フォームの項目が増える度にhooksの実装とインターフェースの両方が肥大化していく
...
- **ロジックとマークアップの共存**: 各render hooks内で、関連するstateとReactNodeが近くに配置される
...
render hooksは、ReactNodeを返すカスタムhooksという単純なパターンです。
