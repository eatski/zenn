...
<!-- 
## SPAってなに？？
*ここからは個人の見解を話します。今までの実演ベース・事実ベースの内容とはテイストが異なりますのでご注意ください。*

SPAを語る上で欠かせない概念。それは「居場所」です。
なぜなら（今までの実装サンプルでお気づきになられたかもしれませんが）SPAはViewの変更に`URL`を同期させることによって実現されるからです。
ページ全体のリロードなしでViewを動的に動かすだけならライブラリは必要ありません。
ページの中身をDOM APIで大幅に書き換えて、ユーザーに「あ、なんか自分は違う居場所にいるな」という気持ちにさせればいいのです。
URLとユーザーが体感する「居場所」を同期させることに意味があるからこそ、SPAが流行り関連ライブラリが多数登場したのです。
Webアプリケーションのユーザー体験を形作る上で「居場所」というのは -->
